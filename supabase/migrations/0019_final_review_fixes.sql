-- ============================================================
-- Migration 0019: تصليحات المراجعة الأخيرة
-- ============================================================

-- ------------------------------------------------------------
-- 1) مسح النسخة القديمة من create_invoice (6 باراميترات، من غير
--    p_treatment_plan_id). لما ضفنا الباراميتر السابع في 0017،
--    بوستجريس ماستبدلش النسخة القديمة دي - عمل نسخة جديدة جنبها
--    لأن عدد الباراميترات اتغيّر. دلوقتي بنشيل القديمة عشان
--    يبقى فيه نسخة واحدة بس، صح ونضيفة.
-- ------------------------------------------------------------
drop function if exists create_invoice(uuid, uuid, uuid, numeric, text, jsonb);


-- ------------------------------------------------------------
-- 2) create_invoice: نتأكد إن خطة العلاج (لو اتبعتت) فعلاً بتاعة
--    نفس العيان بتاع الفاتورة، قبل ما نربطهم ببعض. باقي الدالة
--    زي ما هي بالظبط من 0017 (القفل، التحقق من المخزون، والتحقق
--    من عدم تجاوز المبلغ المدفوع).
-- ------------------------------------------------------------
...
begin
  ...
  if p_treatment_plan_id is not null then
    if not exists (
      ...
      where id = p_treatment_plan_id and patient_id = p_patient_id
    ) then
      raise exception 'خطة العلاج المحددة مش تابعة لنفس العيان';
    end if;
  end if;
